...
const Domain = struct {
    ...
    export fn f(
        ...
        const U = checkedMemOp(a.N_VGetDeviceData_Vulkan, .{u}) catch return 1;
        const Udot = checkedMemOp(a.N_VGetDeviceData_Vulkan, .{u_dot}) catch return 1;
            ...
            for (0..@intCast(p.ny)) |iy| {
                for (0..@intCast(p.nx)) |ix| {
                    if (ix == 0 or ix == p.nx - 1 or iy == 0 or iy == p.ny - 1 or iz == 0 or iz == p.nz - 1) {
                        ...
                        const ui: a.sunrealtype = U[@intCast(i)];
                        Udot[@intCast(i)] = @floatCast(c1x * @as(a.sunrealtype, U[@intCast(x_prev)]) + c2x * ui + c1x * @as(a.sunrealtype, U[@intCast(x_next)]) +
                            c1y * @as(a.sunrealtype, U[@intCast(y_prev)]) + c2y * ui + c1y * @as(a.sunrealtype, U[@intCast(y_next)]) +
                            c1z * @as(a.sunrealtype, U[@intCast(z_prev)]) + c2z * ui + c1z * @as(a.sunrealtype, U[@intCast(z_next)]));
                    }
                    i += 1;
                    x_prev += 1;
                    x_next += 1;
        ...
        a.N_VMarkHostNeedsUpdate_Vulkan(u_dot);
...
const Solver = struct {
    ...
    fn init(allocator: std.mem.Allocator, io: std.Io, domain: *Domain, opts: Options) !*Solver {
        ...
        solver.u = try checkedMemOp(a.N_VNew_Vulkan, .{ domain.nodes, solver.ctx });
